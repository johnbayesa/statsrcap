...
```{r Q1}
m1 <- lm(data = ames_train, log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope)

m2<-stepAIC(m1, direction = "backward", k = 2)
m3<-stepAIC(m1, direction = "backward", k = log(1000))
summary(m2)
summary(m3)
```
...
summary(lm(data=ames_train, log(price)~Bedroom.AbvGr))
summary(lm(data=ames_train, log(price)~Bedroom.AbvGr + log(area)))
```
...
```{r Q3}
m1.resids <- resid(lm(data=ames_train, log(price)~log(area)))
ames_train1 <- cbind(ames_train, m1.resids)
ggplot(data=ames_train1, aes(Neighborhood, m1.resids)) + geom_bar(stat = "summary", fun.y = "mean") + coord_flip()
...
```{r Q4}
ames_train1$m1.sq.resids <- ames_train1$m1.resids^2
ggplot(data=ames_train1, aes(Neighborhood, m1.sq.resids)) + geom_bar(stat = "summary", fun.y = "mean") + coord_flip()
...
```{r Q5}
ames_train2 <- ames_train[names(ames_train) %in% c("Bsmt.Qual", "Overall.Qual", "Garage.Qual")]
ames_train2 %>% filter(!complete.cases(ames_train2)) %>% summarise(n())
...
```{r Q7}
m1 <- lm(data=ames_train, log(price)~Overall.Cond + Overall.Qual)
m1.predict <- predict(m1)
ames_train1 <- cbind(ames_train, m1.predict)
ames_train1 %>% group_by(MS.SubClass) %>% summarise(median=median(m1.predict)) %>% arrange(desc(median))
...
```{r Q8}
m1 <- lm(data=ames_train, log(price)~Overall.Cond + Overall.Qual)
m1.hvals <- hatvalues(m1)
ames_train1 <- cbind(ames_train1, m1.hvals)
#268
```
...
```{r Q11}
ames_train %>% filter(Sale.Condition=="Normal") %>% summarise(n()) / 1000
...
```{r Q12}
summary(lm(data=ames_train, log(price)~log(area)))
summary(lm(data=ames_train%>%filter(Sale.Condition=="Normal"), log(price)~log(area)))
