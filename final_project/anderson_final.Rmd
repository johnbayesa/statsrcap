...
```{r}
ggplot(data = ames_train, aes(Neighborhood, price)) + geom_boxplot() + scale_y_continuous(labels = dollar) + coord_flip()
```
...
ames_test <- ames_test %>% mutate(resids = price - predicted)
ames_test %>% group_by(Neighborhood) %>% summarise(mean(resids))
```
