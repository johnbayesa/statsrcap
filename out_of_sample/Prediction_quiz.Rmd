...
```{r Q1}
model.AIC.predict <- predict(model.AIC, ames_test)
mean(exp(model.AIC.predict))
```
...
```{r Q2}
model.AIC.rmse <- sqrt(mean((ames_test$price - exp(model.AIC.predict))^2))
model.AIC.rmse.train <- sqrt(mean((ames_train$price - exp(model.AIC$fitted.values))^2))

model.full.rmse.train <- sqrt(mean((ames_train$price - exp(model.full$fitted.values))^2))

model.full.rmse.train
rmse.full

model.AIC.rmse
model.AIC.rmse.train
...
```{r Q4}
model.BIC <- stepAIC(model.full, direction = "backward", k = log(834))
model.BIC.predict <- predict(model.BIC, ames_test, interval = "prediction")
model.BIC.coverage <- mean(ames_test$price > exp(model.BIC.predict[,"lwr"]) & ames_test$price < exp(model.BIC.predict[,"upr"]))
model.BIC.coverage
```
...
```{r Q5}
model.bpm.predicts <- predict(model.bas, newdata = ames_test, estimator="BPM")
model.BPM.rmse <- sqrt(mean((exp(model.bpm.predicts$fit)-ames_test$price)^2))

model.BMA.predicts <- predict(model.bas, newdata = ames_test, estimator="BMA")
model.BMA.rmse <- sqrt(mean((exp(model.BMA.predicts$fit)-ames_test$price)^2))
